...
To start baking, you must start your node with the baker keys. You can run a baker node with keys generated in the |mw-gen2| or |mw-gen1| on :ref:`Ubuntu <run-node-ubuntu>`, :ref:`Windows <run-node-windows>`, :ref:`Docker<run-a-node>`, or :ref:`macOS <run-node-macos>`. You can also choose to have a third-party node runner run a node for you if you do not want to run the node yourself.

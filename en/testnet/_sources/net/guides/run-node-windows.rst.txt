...
#. If the node is running properly and reporting to the network dashboards, you can see it on the dashboard: https://dashboard.testnet.concordium.com/
...
#. For the testnet version of the Desktop Wallet, you must connect to a testnet node. In the **Address** field, enter *127.0.0.1* and in the **Port field** enter *10001*.
   ...
   .. image:: ../images/run-node/Node-setup-win-8.png
        :width: 50%
...
For detailed information about all the configuration options, see :ref:`Concordium node nunner service configuration <node-runner-service-configuration>`.

Configure a node as baker
-------------------------
...
The file ``testnet.log`` contains the latest logs, with ``testnet.0.log`` and ``testnet.1.log`` containing progressively older logs. The log files are rolled when the latest log file exceeds 50 MB. This means that ``testnet.0.log`` is renamed to ``testnet.1.log`` (replacing the old file if present), ``testnet.log`` is renamed to ``testnet.0.log``, and a new, empty ``testnet.log`` is created.
...
#. Go to the testnet dashboard to monitor when the node has caught up with its peers on the blockchain. You do so by comparing the finalized length of the chain with the length of your node. If they match, your node has caught up.

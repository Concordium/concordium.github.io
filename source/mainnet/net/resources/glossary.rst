...
A random value generated by a Rust server or backend for a dApp, which is passed with the statement to prove to the wallet. The prover (wallet) must use it to generate their next message.

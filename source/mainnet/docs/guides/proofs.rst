...
.. note::
    Accounts created with the legacy backup file system (no longer possible to create) cannot generate or use zero-knowledge (zk) ID proofs. These existing accounts remain on the network but cannot perform zk proof checks in dApps, including verifications such as age, nationality, or similar attributes.

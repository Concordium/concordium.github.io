...
The most basic transaction is the CCD transfer that is used to send CCD from one account to another. However, there are several :ref:`transaction types<transaction-types-overview>` on the Concordium blockchain. Change
